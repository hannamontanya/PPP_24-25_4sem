...
def client_handling(conn, addr):
    ...
    while not stop_server_flag.is_set(): # Проверка флага завершения
        try:
            data = SizeProtocol.recv(conn)
            if not data:
                break

            if data.startswith("ADD"): # Обработка команды ADD
                _, prog_name = data.split(maxsplit=1)
                if not os.path.exists(prog_name): # Проверка существования программы
                    SizeProtocol.send(conn, f"Ошибка: программа '{prog_name}' не найдена.")
                    continue

                # Проверка, есть ли активный поток для этой программы
                is_currently_running = any(
                    t.is_alive() and getattr(t, '_args', None) and t._args[0] == prog_name
                    for t in all_threads
                )

                if not is_currently_running:
                    # Добавление/обновление записи в словаре
                    ...
                    saving()
                    SizeProtocol.send(conn, f"Программа '{prog_name}' добавлена и запущена.")
                else:
                    SizeProtocol.send(conn, f"Программа '{prog_name}' уже запущена в данный момент.")

            elif data.startswith("GET"): # Обработка команды GET
                _, prog_name = data.split(maxsplit=1)
                if prog_name in all_progs: # Проверка существования программы
                    ...
                    if not combined_output.strip():
                        combined_output = f"=== {prog_name} ===\nПрограмма еще не выполнялась или вывод пуст.\n"
                    
                    SizeProtocol.send(conn, combined_output)
            else:
                SizeProtocol.send(conn, "Неизвестная команда.")
